...
// SDKServer wraps an in-process MCP server registry.
...
func (s *SDKServer) AddTool(tool *gomcp.Tool, handler gomcp.ToolHandler) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.tools[tool.Name] = &sdkTool{tool: tool, handler: handler}
}
...
func convertCallToolResultToMap(result *gomcp.CallToolResult) map[string]any {
	if result == nil {
	...
	for _, c := range result.Content {
		...
		case *gomcp.ImageContent:
			content = append(content, map[string]any{"type": "image", "data": v.Data, "mimeType": v.MIMEType})
		case *gomcp.AudioContent:
			content = append(content, map[string]any{"type": "audio", "data": v.Data, "mimeType": v.MIMEType})
		case *gomcp.ResourceLink:
			content = append(content, map[string]any{"type": "resource_link", "uri": v.URI, "name": v.Name})
		case *gomcp.EmbeddedResource:
			if v.Resource != nil {
				content = append(content, map[string]any{"type": "resource", "resource": map[string]any{"uri": v.Resource.URI, "mimeType": v.Resource.MIMEType, "text": v.Resource.Text}})
			}
		}
	}

	resultMap := map[string]any{"content": content}
	if result.IsError {
...
func goTypeToJSONSchema(goType string) *jsonschema.Schema {
	...
	default:
		if len(goType) > 2 && goType[:2] == "[]" {
			return &jsonschema.Schema{Type: "array", Items: goTypeToJSONSchema(goType[2:])}
		}
		return &jsonschema.Schema{Type: "string"}
	}
}
...
func ErrorResult(message string) *gomcp.CallToolResult {
	return &gomcp.CallToolResult{Content: []gomcp.Content{&gomcp.TextContent{Text: message}}, IsError: true}
}

// ImageResult creates a CallToolResult with image content.
func ImageResult(data []byte, mimeType string) *gomcp.CallToolResult {
	return &gomcp.CallToolResult{Content: []gomcp.Content{&gomcp.ImageContent{Data: data, MIMEType: mimeType}}}
}

// NewTool creates an mcp.Tool with the given parameters.
func NewTool(name, description string, inputSchema *jsonschema.Schema) *gomcp.Tool {
	return &gomcp.Tool{Name: name, Description: description, InputSchema: inputSchema}
}

// ParseArguments unmarshals CallToolRequest arguments into a map.
func ParseArguments(req *gomcp.CallToolRequest) (map[string]any, error) {
